...
class TelnetConnection(telnetlib.Telnet):
    ...
    def execute_command_ex(self,command,command1='q',loglevel=None, strip_prompt=False):
        buff = ''
        self.read()
        prompt, regexp1 = self._prompt
        self.write(command, loglevel)
        time.sleep(1)
        buff = self.read_until_regexp(prompt,'--More--','--MORE--')
        if buff.find('--More--') != -1 or buff.find('--MORE--')!= -1:
           self.write_bare(command1)
           time.sleep(1)
           self.read_until_prompt(loglevel, strip_prompt)
        return buff
                
    def execute_command_showAll(self,command,command1=" ",loglevel=None, strip_prompt=False):
        buff = ''
        self.read()
        prompt, regexp1 = self._prompt
        self.write(command, loglevel)
        time.sleep(1)
        buff = self.read_until_regexp(prompt,'--More--','--MORE--')
        while buff.find('--More--') != -1 or buff.find('--MORE--')!= -1:
           self.write_bare(command1)
           time.sleep(1)
           temp=self.read_until_regexp(prompt,'--More--','--MORE--')
           buff = buff+temp          
           if temp.find('--More--') == -1 and temp.find('--MORE--') == -1:
           	   break
        return buff
